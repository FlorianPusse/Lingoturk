package models.Questions.LinkingV1Experiment.Prolific;


import com.amazonaws.mturk.service.axis.RequesterService;
import models.Groups.AbstractGroup;
import models.Groups.LinkingV1Experiment.LinkingV1PoolGroup;
...
@Entity
@Inheritance
@DiscriminatorValue("LinkingV1Experiment.Prolific.LinkingGroup")
public class LinkingGroup extends AbstractGroup {
    ...
    public static Result publishProlific(int expId) throws SQLException {
        ...
        for (AbstractGroup p : expModel.getParts()) {
            LinkingV1PoolGroup tmp = (LinkingV1PoolGroup) p;
        ...
        for (int i = 0; i < groupOfCombinations.size(); i++) {
            ...
            if (i % MAX_GROUP_COLLECTION_SIZE == 0 && (i != 0)) {
                LingoExpModel exp = new LingoExpModel(expModel.getName() + "_publish_" + (groupCollections.size() + 1), expModel.getDescription(), expModel.getAdditionalExplanations(), expModel.getNameOnAmt(),"LinkingV1Experiment");
                exp.save();
    ...
    public static Result renderLinkingProlific(int expId) {
        return ok(views.html.ExperimentRendering.LinkingV1Experiment.linkingExperimentProlific.render(expId));
    }
